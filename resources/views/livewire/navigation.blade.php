<header class="bg-trueGray-700 sticky top-0" style="z-index: 900;" x-data="dropdown()">
    <div class="container flex items-center h-16 justify-between md:justify-start">
        <a :class="{'bg-opacity-100 text-cyan-500' : open}" x-on:click="show()"
    ...
    <nav id="navigation-menu" :class="{'block': open, 'hidden': !open}" x-show="open"
        ...
        <div class="container h-full hidden md:block">
            <div x-on:click.away="close()" class="grid grid-cols-4 h-full relative">
                <ul class="bg-white">
                    ...
                    <li class="navigation-link text-trueGray-500 hover:bg-cyan-500 hover:text-white">
        ...
        <div class="bg-white h-full overflow-y-auto">
            ...
            <div class="container bg-gray-100 py-3 mb-2">
                @livewire('search')
            </div>

            <ul>
                @foreach ($categories as $category)
                <li class="text-trueGray-500 hover:bg-cyan-500 hover:text-white">
                    <a href="{{route('categories.show', $category)}}" class="py-2 px-4 text-sm flex items-center">
            ...
            <a href="{{ route('profile.show') }}"
                class="py-2 px-4 text-sm flex items-center text-trueGray-500 hover:bg-cyan-500 hover:text-white">
                <span class="flex justify-center w-9">
